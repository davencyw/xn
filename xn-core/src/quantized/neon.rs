...
pub(crate) unsafe fn sgemm_q8_0_q8_0_raw(
    ...
    let blas = TinyBlasQ0Arm { k, a, lda, b, ldb, c, ldc, ith, nth };
...
pub(crate) fn vec_dot_q6k_q8k(n: usize, xs: &[BlockQ6K], ys: &[BlockQ8K]) -> Result<f32> {
    ...
    unsafe {
        ...
        for (x, y) in xs.iter().zip(ys.iter()) {
            ...
            let q6scales =
                int16x8x2_t(vmovl_s8(vget_low_s8(scales)), vmovl_s8(vget_high_s8(scales)));
...
pub(crate) fn vec_dot_q5k_q8k(n: usize, xs: &[BlockQ5K], ys: &[BlockQ8K]) -> Result<f32> {
    ...
    unsafe {
        ...
        for (x, y) in xs.iter().zip(ys.iter()) {
            ...
            let q8sums =
                vpaddq_s16(vld1q_s16(y.bsums.as_ptr()), vld1q_s16(y.bsums.as_ptr().add(8)));
...
pub(crate) fn vec_dot_q4k_q8k(n: usize, xs: &[BlockQ4K], ys: &[BlockQ8K]) -> Result<f32> {
    ...
    unsafe {
        ...
        for (x, y) in xs.iter().zip(ys.iter()) {
            ...
            let mins8 = vld1_u32(
                [utmp[1] & KMASK1, ((utmp[2] >> 4) & KMASK2) | (((utmp[1] >> 6) & KMASK3) << 4)]
                    .as_ptr(),
            );
            utmp[1] = (utmp[2] & KMASK2) | (((utmp[0] >> 6) & KMASK3) << 4);
            utmp[0] &= KMASK1;
