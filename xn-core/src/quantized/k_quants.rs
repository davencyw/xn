...
fn matmul_q8_0_sgemm(
    ...
    // pool is shared with another busy thread, and costs ~3.4% otherwise. The single-worker
    // fast path this used to keep here is now `dispatch`'s job: it runs the closure inline as
    // `(0, 1)` when the pool has one worker, is contended, or we are nested inside a job.
