...
impl HParams {
    fn read<R: std::io::Read>(reader: &mut R) -> Result<Self> {
        ...
        Ok(Self { n_vocab, n_embd, n_mult, n_head, n_layer, n_rot, ftype })
    }
}
...
pub fn qtensor_from_ggml(
    ...
    match ggml_dtype {
        ...
        GgmlDType::Q8_0 => {
            let plain = from_raw_data::<k_quants::BlockQ8_0>(raw_data, size_in_bytes, dims)?;
            Ok(super::repack::maybe_interleave(plain))
        }
...
impl Content {
    pub fn read<R: std::io::Seek + std::io::Read>(reader: &mut R) -> Result<Content> {
        ...
        Ok(Self { magic, hparams, vocab, tensors })
    }
