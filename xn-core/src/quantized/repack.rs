...
unsafe fn tile<const RM: usize>(
    ...
    unsafe {
        ...
        for (r, acc) in acc.iter().enumerate() {
            for c in 0..NCOLS {
                *out.add(r * out_stride + c) = acc[c];
            }
        }
    }
}

/// Whether to interleave at all. `XN_Q8_REPACK=0` keeps the stored layout, which is the
/// escape hatch if a platform ever turns out to prefer the row-tiled `sgemm` path.
fn enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| !matches!(std::env::var("XN_Q8_REPACK").as_deref(), Ok("0")))
}

/// Move a freshly loaded `q8_0` tensor into the interleaved layout when its shape allows.
///
/// Anything that does not qualify is returned untouched, so this is safe to call on every
/// `q8_0` tensor a file contains.
pub fn maybe_interleave(t: super::QTensor) -> super::QTensor {
    if !enabled() || t.dtype() != GgmlDType::Q8_0 || !is_eligible(t.shape().dims()) {
        return t;
    }
    let dims = t.shape().dims();
    let (n, k) = (dims[0], dims[1]);
    let super::QStorage::Cpu(storage) = &t.storage;
    // `from_raw_data` always produces a `Vec<BlockQ8_0>` for this dtype.
    let plain = unsafe {
        std::slice::from_raw_parts(
            storage.as_ptr() as *const BlockQ8_0,
            storage.storage_size_in_bytes() / std::mem::size_of::<BlockQ8_0>(),
        )
    };
    match Q8_0x4::from_q8_0(plain, n, k) {
        Ok(packed) => super::QTensor {
            storage: super::QStorage::Cpu(Box::new(packed)),
            shape: t.shape().clone(),
        },
